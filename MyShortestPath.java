...
class Graph
{
    ...
    void addEdge(int v,int w,int distance)
    {
        ...
        adj[w].add(new Edge(v,distance));
    }
 
    // computes shortest path of each vertex from vertex s using breadth first traversal
    int[] shortestPathBFS(int s)
    {
        ...
        while (queue.size() != 0)
        {
            ...
            while (i.hasNext())
            {
                ...
                //if (!visited[n]){
                    //visited[n] = true;
                    int newdist = distance[s] + e.distance;
                    if(newdist < distance[n]){ //if new distance is less than old distance
                    	distance[n]=newdist; //update distance to new lesser value
                    	queue.add(n); //re-explore all neighbours of the node
                    }
